...
async def greeting(message: types.Message, state: FSMContext) -> None:
    ...
    await bot.send_message(
        message.from_user.id, "Приветствую!", reply_markup=keyboards.GREETING
    )
...
async def start_dialog(message: types.Message, state: FSMContext) -> None:
    ...
    await bot.send_message(
        message.from_user.id, "Пожалуйста, выберите предпочитаемую тему задач"
    )
...
async def get_topic_name(message: types.Message, state: FSMContext) -> None:
    ...
    await bot.send_message(
        message.from_user.id, "Пожалуйста, выберите предпочитаемую сложность задач"
    )
...
async def get_complexity_name(message: types.Message, state: FSMContext) -> None:
    ...
    try:
        ...
        list_of_problems = bot_bd.get_list_of_problems(
            data["topic_name"], data["complexity"]
        )
        ...
        await bot.send_message(
            message.from_user.id, result, reply_markup=keyboards.GREETING
        )

    except Exception:
        await bot.send_message(
            message.from_user.id,
            "Что-то не так, возможно, вы ошиблись при вводе данных",
...
def register_main_handlers(dp):
    ...
    dp.register_message_handler(
        get_topic_name,
        content_types=["text"],
        state=GetProblemListFSM.waiting_topic_name,
    ),
    dp.register_message_handler(
        get_complexity_name,
        content_types=["text"],
        state=GetProblemListFSM.waiting_complexity,
    )
