...
def all_pages_parser(url: str, headers: dict) -> list[dict]:
    ...
    for i in range(1, num_of_pages + 1):
        ...
        for problem in problems:
            ...
            record = {
                'number': problem.find_all('td')[0].text.strip() if problem.find_all('td')[0] else 'None',

                'name': problem.find_all('td')[1].find_all('div')[0].text.strip()
                if problem.find_all('td')[1] and problem.find_all('td')[1].find_all('div')[0] else 'None',

                'topic': ', '.join(map(lambda x: x.text.strip(),
                                       problem.find_all('td')[1].find_all('div')[1].find_all('a')))
                if problem.find_all('td')[1] and problem.find_all('td')[1].find_all('div')[1] else 'None',

                'complexity': problem.find_all('td')[3].text.strip() if problem.find_all('td')[3] else 'None',

                'solutions_num': problem.find_all('td')[4].text.strip() if problem.find_all('td')[4] else 'None'
            }
